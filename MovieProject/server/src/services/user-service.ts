...
export const UserUpdateSchema = z.object({
    ...
    email: z.string().refine(value => value.includes('@'), 'Email must contain @').optional(),
...
export class UserService {
    async register(args: UserInput) {
        ...
        {
            if (error instanceof UniqueViolationError &&  error.constraint == 'users_email_unique')
            {
                throw new EmailAlreadyExistsError;
            }

            if (error instanceof UniqueViolationError &&  error.constraint == 'users_username_unique')
            {
